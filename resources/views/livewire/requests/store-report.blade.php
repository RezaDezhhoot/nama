                ...
                <div class="col-12">
                    <table class="table table-striped table-bordered">
                        ...
                        <tr>
                            ...
                            <th>هزینه پیشنهادی مرحله دوم <span>{{ $request->plan_type === \App\Enums\PlanTypes::UNIVERSITY ? 'معاونت دانشجویی' : 'معاونت اجرایی مساجد' }}</span></th>
                ...
                @if($report->status !== \App\Enums\RequestStatus::DONE)
                    ...
                    <div class="col-12">
                        ...
                        @if($report->step === \App\Enums\RequestStep::APPROVAL_EXECUTIVE_VICE_PRESIDENT_MOSQUES)
                            <x-admin.forms.input type="number"  id="offer_amount" label="قیمت پیشنهادی توسط {{ \App\Enums\OperatorRole::EXECUTIVE_VICE_PRESIDENT_MOSQUES->label($type) }} مرحله دوم(ریال)" wire:model.defer="offer_amount"/>
                        @elseif($report->step === \App\Enums\RequestStep::APPROVAL_DEPUTY_FOR_PLANNING_AND_PROGRAMMING)
                            @if($request->single_step)
                                <x-admin.forms.input help="پس از تایید صد درصد هزینه فوق برای درخواست کننده واریز خواهد شد" type="number"  :required="true" id="final_amount" label="قیمت نهایی تایید شده توسط {{ \App\Enums\OperatorRole::DEPUTY_FOR_PLANNING_AND_PROGRAMMING->label($type) }} مرحله دوم(ریال)" wire:model.defer="final_amount"/>
                            @else
                                <x-admin.forms.input help="پس از تایید پنجاه درصد هزینه فوق برای درخواست کننده واریز خواهد شد" type="number"  :required="true" id="final_amount" label="قیمت نهایی تایید شده توسط {{ \App\Enums\OperatorRole::DEPUTY_FOR_PLANNING_AND_PROGRAMMING->label($type) }} مرحله دوم(ریال)" wire:model.defer="final_amount"/>
                            @endif
                        @endif
                        <x-admin.forms.text-area dir="rtl" id="comment" :required="true" label="کامنت" wire:model.defer="comment"/>
                        <x-admin.forms.dropdown :data="$data['status']" :required="true" id="status" label="وضعیت درخواست" wire:model.live="status"/>
